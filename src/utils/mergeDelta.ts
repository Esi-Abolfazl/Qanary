 ...
 * snapshot's `overall`, `cut_off` and `settled` from the delta — the backend already recomputed
 * all of them. An unknown list or service id is a no-op: returns the input snapshot unchanged
 * (e.g. a delta racing a config reset).
 */
export function mergeDelta(snap: Snapshot, d: ServiceDelta): Snapshot {
  ...
  return { ...snap, lists, overall: d.overall, cut_off: d.cut_off, settled: d.settled };
}
