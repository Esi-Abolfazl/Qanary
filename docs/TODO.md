...
- [x] Notification sound volume — one level for all alert sounds (`notify_volume`, 0–100 in steps of 25, default 100), slider in the Critical-list alerts card. Volume 0 clears the three Sound toggles; the rule lives in `store::normalize_alerts` (load, import, every write) so a `{volume: 0, sound: true}` config can't silently swallow an outage. Does not touch the OS banners (ADR-0026)
